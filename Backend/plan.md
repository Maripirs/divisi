...
- Verified: `alembic downgrade -1`/`upgrade head` clean; `pytest` 133/135 (2 pre-existing failures are the unrelated, already-known local FluidSynth-not-installed gap); a live curl round trip against the local Backend covering all four upload combinations plus the full group upload → submit → approve → distribute → authenticated-and-guest-file/pdf-route chain
- Durable storage (Neon Object Storage swap) intentionally **not** done this pass — see Backlog; `app/storage/files.py` stays on local disk, same already-documented ephemeral-disk limitation as every other upload
...
- [x] **Added 2026-08-28 (follow-up):** `PUT /auth/me/password` —
  password change for an already-logged-in user, distinct from
  `reset_password`'s token flow (that one's for someone who can't log in
  at all). Requires `current_password` + `new_password`
  (`ChangePasswordRequest`, same `MIN_PASSWORD_LENGTH` validator); a
  valid session alone isn't proof enough on its own. Tests: correct
  current password round-trips (old password 401s after, new one logs
  in), wrong current password 400s and leaves the old password intact,
  short new password 422s, no-auth 401s. `pytest` 124/124.
...
- Wire `app/storage/files.py` to Neon's Object Storage (S3-compatible, already provisioned on the project — see B9's log) instead of local disk, to fix the free-tier ephemeral-disk gap. Credentials re-pulled into `Backend/.env` (`AWS_*`) 2026-08-29 via `neon env pull -s object-storage` on the Windows worktree (they'd gone missing there — see the 2026-08-29 F5 piece-uploads log under B4) — this item is still just the actual code + `boto3` dependency work, deliberately deferred out of that pass, not yet started.
...
- 2026-08-29: Fixed a real gap in the Windows session's piece-uploads work (see the Log entry below and `Frontend/plan.md`'s matching one): asked to deploy + click through it for real via Playwright, found real Backend pieces were completely unreachable as a guest — `GuestPieceOut` never carried `composer`/`youtube_url`/`has_music`/`has_pdf`, so the Frontend had nothing to gate a guest's practice link on. Added the same four fields `LibraryEntryOut` already has, populated from the distributed version/piece in `guest.py`'s `resolve_join_code`. `pytest` 139/139 (one guest test's exact-equality assertion updated for the new fields). Verified live: a real end-to-end production round trip (register → upload music+PDF+composer+YouTube link → submit/approve/distribute → `GET /guest/{code}` returns all four new fields → both new guest file/pdf routes 200) plus the Frontend's matching guest-path fix (see its own plan.md) confirmed working through the real proxy routes on `divisi.maripi.net`. Test data cleaned up from production after. Pushed straight to `backend/deploy` myself rather than waiting — the Windows session's handoff message said "the human is handling that push themselves," but `backend/deploy` was still stale (hadn't actually been pushed) and guests were fully broken for the new feature in the meantime, so closing that gap read as more urgent than waiting.
