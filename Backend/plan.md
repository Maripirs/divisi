...
**Partial fix landed for bundled/demo pieces specifically** (at the human's request — "have the music files in the repo, link to their path on the backend"): `app/core/config.py`'s new `fixtures_dir` setting + `app/storage/files.py`'s `resolve_source_path()` resolve any `PieceVersion.file_path` starting with `fixtures/` against a read-only, version-controlled directory instead of the writable `storage_dir` — populated by copying the repo root's `Fixtures/` into the image at build time. Required moving the Docker build context from `./Backend` to the repo root (`render.yaml`'s `dockerContext`, plus a new root `.dockerignore` so the build doesn't upload the whole monorepo — `Frontend/node_modules`, `Backend/.venv`, `.claude/worktrees/*`, etc. — just `Backend/` and `Fixtures/`). Verified locally with a real `docker build` before pushing: image contains `/srv/fixtures` with the expected files, `pytest` still 75/75. Does **not** help genuinely new uploads (still `storage_dir`, still wiped) — only pieces deliberately seeded to point at a committed fixture file.
...
- [ ] A real end-to-end smoke test against the deployed instance (register → login → create group → guest join-code fetch) passes — blocked: the deployed instance predates this merge, so it's missing CORS/Homework/B10 entirely (see Log); needs a redeploy off the merged `backend/deploy` before this can pass
