...
- 2026-08-31: **F14 build in progress** (running breadcrumb, updated per task; resume from here after a `/clear` — `git log --grep '^F14' -1` is the latest committed task). Engine + plan already committed. Checklist state:
  ...
  - [ ] Task 3b (NEXT) — duration change. New `EditableScore.setDuration(index, {type, dots})`: rewrite `<type>`/`<dot>`/`<duration>` against the measure's `<divisions>`, then re-fit the measure (absorb the delta into a following rest in the same voice, refuse if it overfills the bar). Tuplets out of scope. Toolbar values whole..16th + dot, digit keys 1-6 and `.`.
  - [ ] Task 3c — key / clef / per-note accidental. Per-note accidental is nearly contained (`<pitch><alter>` + `<note><accidental>` in DTD order, changes sounding pitch so it pairs with the pitch ops). Key sig (`<attributes><key><fifths>`) and clef (`<attributes><clef>`) need a UX decision: edit the piece's initial value vs. insert a change at the selected measure, plus create the `<attributes>` subtree in DTD order when absent.
  - [ ] Tasks 4-9 — MusicXML export, save action, unsaved-changes guard (reads `dirty`), entry points, i18n sweep (incl. removing now-unused `piece_editor_coming_soon`), final `check`/`build`.
