...
- 2026-08-31: **F14 build in progress** (running breadcrumb, updated per task; resume from here after a `/clear`). Engine + plan already committed. Checklist state:
  - [x] Task 1 — editor route `/piece/[id]/edit` (`ssr: false`) + access-gated server `load`. Commit `5511aba`.
  - [x] Task 2 — load the track's music file into the editable model + read-only render. `$lib/musicxml/editableScore.ts` (production port of the spike's `EditableScore`), `$lib/musicxml/loadEditableScore.ts` (sniffs MIDI/MusicXML, `.mxl` rejected, MIDI goes through `convertAllParts` first), `$lib/components/EditorScoreView.svelte` (dedicated read-only OSMD mount, not `ScoreView`). Commit `787e5b9`.
  - [ ] Task 3 (NEXT) — editing surface: click a note (part-aware hit-test via OSMD `GraphicSheet.GetNearestNote` + `EditableScore.findByOnset`), then pitch/duration/delete/key-clef ops with keyboard + click, re-serialize + re-render per edit.
  - [ ] Tasks 4-9 — MusicXML export, save action, unsaved-changes guard, entry points, i18n sweep (incl. removing now-unused `piece_editor_coming_soon`), final `check`/`build`.
