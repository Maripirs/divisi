...
/** B30 / Backend B30: a guest releases an expressed interest, either their
 * own or (if they're the rider post's owner) someone else's. Same shape as
 * `join/[code]/carpool/claims/[claimId]/+server.ts`'s `DELETE`: `local_id`
...
export const DELETE: RequestHandler = async ({ params, url, cookies, locals, fetch }) => {
	...
	if (!interestId) return json({ ok: false, error: 'server' as const });
	...
	if (locals.token) {
		try {
			await backendFetch(
				locals.token,
				`/carpool/interests/${encodeURIComponent(interestId)}`,
				{ method: 'DELETE' },
				fetch
			);
	...
	const url_ = new URL(`${PUBLIC_API_BASE_URL}/carpool/interests/${encodeURIComponent(interestId)}`);
