<script lang="ts">
	...
	function focusSeam(index: number): void {
		if (seams.length === 0 || !score) return;
		seamAt = ((index % seams.length) + seams.length) % seams.length;
	...
	function goToPrevSeam(): void {
		// From the cold state (`seamAt === -1`), step straight to the last seam:
		// `focusSeam(-2)` would wrap to seam 1, skipping seam 0 and the last.
		focusSeam(seamAt < 0 ? seams.length - 1 : seamAt - 1);
	}
	...
	// `reviewPagesRaw` is the paged report's page list, each entry carrying the
	// bar count B18 measured for that page (see `reviewPages.ts`). `reviewPages`
	// lays those counts end to end into live 0-based measure ranges off the
	// current model, keyed on `workingXml` exactly like `seams` above: a
	// *running cumulative sum* of `measureCount`, not each page's fixed
	// `startMeasure`. So an in-editor insert / splice or a re-run that grows one
	// page's bar count (both patch `reviewPagesRaw` below) shifts every later
	// page's highlight with it. For a fresh B18 report the cumulative sum equals
	// `startMeasure - 1` (B18 tiles the merge with no gaps or overlaps), so the
	...
	const reviewPages = $derived.by(() => {
		...
		return reviewPagesRaw.map((p) => {
			const startIndex = Math.min(Math.max(0, running), lastIndex);
			running += Math.max(0, p.measureCount);
			...
			return {
				...
				startIndex,
				endIndex,
				onsetWholeNotes: score!.measureOnset(startIndex + 1)
			};
		});
	});
	...
	// This draft was generated by a paged OMR run (so a page review is meant to
	// happen) but the page list failed to map. Resolving seams alone must not
	// be allowed to stand in for the page pass, so Publish stays blocked and the
	// review bar says why.
	const reviewPagesUnavailable = $derived(!!data.pagedReportJobId && reviewPagesView.length === 0);

	// Every page approved or (deliberately) skipped unlocks the Seams step;
	// every page *approved* (skips don't count) is half of the Publish gate. An
	// empty page list counts as cleared / approved only when there was never a
	// paged page review to do (`!reviewPagesUnavailable`); `[].every()` being
	// `true` must not silently open the gate when the list should have been
	// there.
	const allPagesCleared = $derived(
		!reviewPagesUnavailable &&
	...
	// F19 rework: during page/seam review the editor trims its own chrome to one
	// row so the score + PDF panes get the height. Publish step restores it.
	const reviewChromeTrim = $derived(reviewEnabled && reviewStep !== 'publish');
	// F19 rework: the per-mode edit rows collapse behind an [Edit] toggle while
	// review trims the chrome; open by default outside review.
	...
	// A page whose B18 offsets are unknown (a pre-B18 report) has only a
	// placeholder bar range, so it gets no band: its approval is an
	// honour-system "I looked" that doesn't need the range.
	const pageBand = $derived.by(() => {
		...
		return p && p.offsetsKnown ? { fromMeasure: p.startIndex, toMeasure: p.endIndex } : null;
	});
	 ...
	 * page. Mirrors `goToNextSeam`'s scroll/select machinery. A page without
	 * known B18 offsets skips the score select + auto-scroll (its range is a
	 * placeholder); the PDF pane still turns to the page. */
	function selectReviewPage(page: number): void {
		selectedReviewPage = page;
		fillBars = 1;
		editNotice = null;
		const target = reviewPagesView.find((p) => p.page === page);
		if (score && target && target.offsetsKnown && target.onsetWholeNotes != null) {
			const near = score.findByOnset(target.onsetWholeNotes, {});
			if (near) {
				selectByIndex(near.index);
				previewSelected();
			...
			scoreView?.scrollCursorIntoView();
			// F19 rework: `pageBand` -> `computePageBand` runs in an effect, so the
			// band rects aren't ready synchronously; defer the score-pane frame.
			tick2(() => scoreView?.scrollPageIntoView());
		}
		if (data.hasPdf) {
			pdfPaneOpen = true;
	...
	function insertPageBars(): void {
		...
		const added = fillBars;
		const applied = applyStructuralEdit(() => score!.insertMeasures(page.startIndex - 1, added));
		if (applied) {
			clearPageAndTouchingSeams(page.page);
			// The page now spans `added` more bars: bump its count so the
			// cumulative tiling in `reviewPages` pushes every later page past
			// the inserted bars.
			reviewPagesRaw = reviewPagesRaw.map((p) =>
				p.page === page.page ? { ...p, ok: true, measureCount: p.measureCount + added } : p
			);
			editNotice = null;
		} else {
	...
	async function rerunReviewPage(): Promise<void> {
		...
		try {
			...
			if (applied) {
				// The report's `ok` / bar count for this page are stale until the
				// next report fetch. Patch them locally so the rail, the approve
				// gate, and `reviewPages`' cumulative tiling (which shifts every
				// later page) all see the fix right away.
				reviewPagesRaw = reviewPagesRaw.map((p) =>
					p.page === page.page
						? { ...p, ok: true, measureCount: result.measure_count }
						: p
				);
				clearPageAndTouchingSeams(page.page);
	...
	// `<note>` via `findByOnset`; Measures mode takes the bar's 0-based index
	// straight from OSMD's graphical measure list (see the hit type), since
	// `findByOnset` can't place a click in a bar where the clicked part has no
	// note (a tacet opening).
	...
	{#if data.access === 'granted'}
		...
		{:else if phase === 'ready'}
				...
				<div class="editor-toolbars">
					...
					{#if reviewChromeTrim}
						<button
							...
							onclick={() => (editToolsOpen = !editToolsOpen)}
						>
							{m.piece_editor_review_edit_tools()}
						</button>
					{/if}
					{#if !reviewChromeTrim || editToolsOpen}
					{#if editMode === 'note'}
			...
			<footer class="transport-bar">
				{#if reviewEnabled}
					{#if reviewPagesUnavailable}
						<div class="review-bar" role="group" aria-label={m.piece_editor_review_panel()}>
							<span class="seam-readout" role="status" aria-live="polite">
								{m.piece_editor_review_pages_unavailable()}
							</span>
						</div>
					{:else if reviewStep === 'pages'}
						{@const cur =
							selectedReviewPage != null
								? reviewPagesView.find((p) => p.page === selectedReviewPage)
								: undefined}
						{@const idx = cur ? reviewPagesView.findIndex((p) => p.page === cur.page) : -1}
						<div class="review-bar" role="group" aria-label={m.piece_editor_review_panel()}>
							<button
								class="review-nav"
								disabled={idx <= 0}
								aria-label={m.piece_editor_review_prev_page()}
								onclick={() => idx > 0 && selectReviewPage(reviewPagesView[idx - 1].page)}
							>‹</button>
							<span class="seam-readout" role="status" aria-live="polite">
								{#if cur}
									{m.piece_editor_review_page_of({ n: cur.page, total: reviewPagesView.length })} - {cur.status ===
									'approved'
							...
							{#if cur}
								{#if cur.status === 'failed'}
									<span class="seam-fill">
										...
										<button class="btn" onclick={insertPageBars} disabled={reRendering || fillBars < 1}>
											{m.piece_editor_seam_fill_button({ count: fillBars })}
										</button>
										<button class="btn" onclick={rerunReviewPage} disabled={rerunning || reRendering}>
								...
								{#each reviewSegments as seg (seg.pages.join(','))}
									{#if seg.pages.length > 1 && seg.pages.includes(cur.page) && seg.pages.every((pg) => reviewPagesView.find((x) => x.page === pg)?.status !== 'failed')}
										<button class="review-link" onclick={() => approveSegment(seg.pages)}>
							...
							<button
								class="review-nav"
								disabled={idx < 0 || idx >= reviewPagesView.length - 1}
								aria-label={m.piece_editor_review_next_page()}
								onclick={() =>
									idx >= 0 &&
									idx < reviewPagesView.length - 1 &&
									selectReviewPage(reviewPagesView[idx + 1].page)}
							>›</button>
							{#if allPagesCleared}
								<button class="review-link" onclick={() => (reviewStep = 'seams')}>
									{m.piece_editor_review_to_seams()}
								</button>
							{/if}
						</div>
					{:else if reviewStep === 'seams'}
						...
						<div class="review-bar" role="group" aria-label={m.piece_editor_review_panel()}>
							<button class="review-link" onclick={() => (reviewStep = 'pages')}>
								{m.piece_editor_review_to_pages()}
							</button>
							<button
								class="review-nav"
								disabled={seams.length === 0}
								aria-label={m.piece_editor_review_prev_page()}
								onclick={goToPrevSeam}
							>‹</button>
							<span class="seam-readout" role="status" aria-live="polite">
								{#if curSeam}
							...
							<button
								...
								disabled={seams.length === 0}
								aria-label={m.piece_editor_review_next_page()}
								onclick={goToNextSeam}
							>›</button>
							{#if allPagesApproved && allSeamsResolved}
								<button class="review-link" onclick={() => (reviewStep = 'publish')}>
									{m.piece_editor_review_to_publish()}
								</button>
							{/if}
						</div>
					{:else}
...
<style>
	...
	/* F19 rework: the [Edit] disclosure toggle shown while review trims the
	   chrome. Reveals the per-mode edit rows, collapsed by default in review. */
	.edit-tools-toggle {
	...
	/* F19 rework: one compact review row in the transport bar: prev/next nav, a
	   status readout, the phase's action, and text links to move between the
	   pages / seams / publish phases. Replaces the tab strip + page-chip rail. */
	.review-bar {
		display: flex;
		align-items: center;
		flex-wrap: wrap;
