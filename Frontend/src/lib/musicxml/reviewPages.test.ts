...
describe('mapReport', () => {
	it('uses B18 start_measure / measure_count when present', () => {
		...
		expect(out).toEqual([
			{ page: 1, ok: true, startMeasure: 1, measureCount: 4 },
			{ page: 2, ok: true, startMeasure: 5, measureCount: 3 },
			{ page: 3, ok: true, startMeasure: 8, measureCount: 6 }
		]);
	});

	it('walks a running 1-based offset when the fields are null (pre-B18 report)', () => {
		const out = mapReport([
			page({ page: 1, measure_count: null }),
			page({ page: 2, measure_count: null }),
			page({ page: 3, measure_count: null })
		]);
		// No counts at all: every page anchors at bar 1.
		expect(out.map((p) => p.startMeasure)).toEqual([1, 1, 1]);
	});
	...
	it('carries a failed page through at count 0, anchored at the next page start', () => {
		...
		expect(out[1]).toEqual({ page: 2, ok: false, startMeasure: 5, measureCount: 0 });
		expect(out[2].startMeasure).toBe(5);
	});
});

describe('pageStatus', () => {
	const p: ReviewPageRaw = { page: 1, ok: true, startMeasure: 1, measureCount: 4 };
	const failed: ReviewPageRaw = { page: 2, ok: false, startMeasure: 5, measureCount: 0 };
